...
(function () {
    ...
    class s{constructor(){this.subscribers=new Set,this.subscribersHot=new Set,this.dispatched=!1,this.hot=!1;}dispatch(s){const t=this.val;this.val=s,this.dispatched=!0,this.subscribers.forEach(i=>i(s,t));}subscribeNext(s){return this.subscribers.add(s),this.notifyHotSubscribers(),{unsubscribe:()=>{this.subscribers.delete(s);}}}subscribe(s){return this.dispatched&&s(this.val),this.subscribeNext(s)}unsubscribe(s){this.subscribers.delete(s),this.notifyHotSubscribers();}notifyHotSubscribers(){this.subscribers.size>0!=this.hot&&(this.hot=this.subscribers.size>0,this.subscribersHot.forEach(s=>s(this.hot)));}subscribeHot(s){return this.subscribersHot.add(s),{unsubscribe:()=>{this.subscribersHot.delete(s);}}}clear(){this.subscribers.clear(),this.subscribersHot.clear();}value(){return this.val}promise(){return new Promise(s=>{const t=this.subscribe((function(i){t.unsubscribe(),s(i);}));})}map(t){const i=new s,e=s=>{i.dispatch(t(s));};return i.subscribeHot(s=>{s?this.subscribe(e):this.unsubscribe(e);}),i}filter(t){const i=new s,e=s=>{t(s)&&i.dispatch(s);};return i.subscribeHot(s=>{s?this.subscribe(e):this.unsubscribe(e);}),i}}function e$1(t,i){const e=new s,r=s=>{e.dispatch(s);};return e.subscribeHot(s=>{s?t.addEventListener(i,r,!1):t.removeEventListener(i,r,!1);}),e}
    ...
    `;
        ...
        init(redraw$, resize$, size$) {
            ...
            this.click$ = e$1(this.canvasDomElement, 'click')
                .map((event) => {
    ...
    `;
        ...
        connectedCallback() {
            e('[data-close]', this.shadowRoot).addEventListener('click', () => {
                this.close();
            });
            ...
            document.documentElement.addEventListener('focusout', (e$1) => {
                if (event.target.closest("gof-info") != null) {
                    e('[data-close]', this.shadowRoot).focus();
                    return false;
                }
            });
        }
    ...
    `;
        ...
        connectedCallback() {
            ...
            this.timer = null;
            this.generation = 0;
            this.generationElement = e('.generation', this.shadowRoot);
            this.collection = this.getCollection();
            this.newShape$ = new s();
            this.nextShape$ = new s();
            this.nextGeneration$ = new s();
            this.resize$ = e$1(window, 'resize');
        }
        init(redraw$, toggle$) {
            this.redraw$ = redraw$;
            this.info$ = e$1(e('#info', this.shadowRoot), 'click');
            toggle$.subscribe(() => {
            ...
            e('#next', this.shadowRoot).addEventListener('click', () => {
                this.nextGeneration$.dispatch();
            });
            this.size$ = e$1(e('#size', this.shadowRoot), 'input')
                .map((event) => Math.round(2 + 38 / 100 * Number(event.target.value)));
            this.size$.dispatch(24);
            const speed = e('#speed', this.shadowRoot);
            ...
            const startStop = e('#start', this.shadowRoot);
